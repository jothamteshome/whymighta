...
async def set(inter):
    ...
    await database.set_bot_text_channel_id(inter.guild.id, inter.channel.id)
...
async def mock(inter):
    ...
    if await database.toggle_mock(inter.guild_id):
...
async def binary(inter):
    ...
    if await database.toggle_binary(inter.guild_id):
