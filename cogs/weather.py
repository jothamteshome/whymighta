...
class Weather(commands.Cog):
    ...
    async def weather(
        ...
        weather_conditions = {
            "thunderstorm": "It is storming right now.",
            "drizzle": "It is drizzling right now",
            "rain": "It is raining right now.",
            "snow": "It is snowing right now.",
            "clear": "It is clear outside right now.",
            "clouds": "It is cloudy right now.",
        ...
        async with ClientSession(trust_env=True) as session:
            async with session.get(
                ...
                "?q={city_name},{state_code},{country_code}&limit=1&appid={api_key}".format(
                    city_name=city,
            ...
            async with session.get(
                "https://restcountries.com/v3.1/alpha/{code}".format(code=country_code)
            ) as code_response:
            ...
            async with session.get(
                ...
                "?lat={lat}&lon={lon}&appid={api_key}&units={unit_type}".format(
                    lat=lat, lon=lon, api_key=weather_api, unit_type=units
                )
            ) as weather_response:
                try:
        ...
        await inter.response.send_message(
            ...
            "{condition}".format(
                ...
                high=high_temp,
                low=low_temp,
                temp_id=temp_identifiers[units],
